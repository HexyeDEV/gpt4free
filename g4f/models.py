...
gpt_4 = Model(name="gpt-4", base_provider="openai", best_provider=Bing)

# Bard
palm = Model(name="palm", base_provider="google", best_provider=Bard)
...
claude_instant_v1 = Model(
    name="claude-instant-v1", base_provider="anthropic", best_provider=Vercel
)

claude_v1 = Model(name="claude-v1", base_provider="anthropic", best_provider=Vercel)

claude_v2 = Model(name="claude-v2", base_provider="anthropic", best_provider=Vercel)

command_light_nightly = Model(
    name="command-light-nightly", base_provider="cohere", best_provider=Vercel
)

command_nightly = Model(
    name="command-nightly", base_provider="cohere", best_provider=Vercel
)

gpt_neox_20b = Model(
    name="EleutherAI/gpt-neox-20b", base_provider="huggingface", best_provider=Vercel
)
...
santacoder = Model(
    name="bigcode/santacoder", base_provider="huggingface", best_provider=Vercel
)

bloom = Model(
    name="bigscience/bloom", base_provider="huggingface", best_provider=Vercel
)

flan_t5_xxl = Model(
    name="google/flan-t5-xxl", base_provider="huggingface", best_provider=Vercel
)

code_davinci_002 = Model(
    name="code-davinci-002", base_provider="openai", best_provider=Vercel
)

gpt_35_turbo_16k = Model(
    name="gpt-3.5-turbo-16k", base_provider="openai", best_provider=Vercel
)

gpt_35_turbo_16k_0613 = Model(name="gpt-3.5-turbo-16k-0613", base_provider="openai")

gpt_35_turbo_0613 = Model(
    name="gpt-3.5-turbo-0613",
    base_provider="openai",
    best_provider=RetryProvider([Aivvm, ChatgptLogin]),
)

gpt_4_0613 = Model(name="gpt-4-0613", base_provider="openai", best_provider=Aivvm)

gpt_4_32k = Model(name="gpt-4-32k", base_provider="openai", best_provider=Aivvm)

gpt_4_32k_0613 = Model(
    name="gpt-4-32k-0613", base_provider="openai", best_provider=Aivvm
)

text_ada_001 = Model(name="text-ada-001", base_provider="openai", best_provider=Vercel)

text_babbage_001 = Model(
    name="text-babbage-001", base_provider="openai", best_provider=Vercel
)

text_curie_001 = Model(
    name="text-curie-001", base_provider="openai", best_provider=Vercel
)

text_davinci_002 = Model(
    name="text-davinci-002", base_provider="openai", best_provider=Vercel
)

text_davinci_003 = Model(
    name="text-davinci-003", base_provider="openai", best_provider=Vercel
)
